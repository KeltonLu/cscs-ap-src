...
namespace CIMSBatch.FTP
{
    ...
    public class FTPFactory
    {
        ...
        public string[] GetFileList(string remotePath, string remFileName)
        {
            ...
            {
                // Legend 2018/04/10 添加路徑與名稱值
                LogFactory.Write("獲取FTP目錄 remotePath:" + remotePath + ", remFileName: " + remFileName + "錯誤：" + ex.ToString(), GlobalString.LogType.ErrorCategory);
                return null;
            }
        }
        #endregion
        ...
        public bool Download(string remotePath, string remFileName, string locPath, string locFileName)
        {
            ...
            try
            {
                ...
                {
                    // Legend 2018/04/10 添加路徑
                    LogFactory.Write("remotePath: " + remotePath + "的檔案 " + remFileName + " 未找到！", GlobalString.LogType.ErrorCategory);
                    return false;
                }
                MKDir(locPath);
            ...
            {
                // Legend 2018/04/10 添加路徑
                LogFactory.Write("remotePath: " + remotePath + ",下載檔案 " + remFileName + " 失敗：" + ex.ToString(),
...
GlobalString.LogType.ErrorCategory);
        ...
        public bool Delete(string remotePath, string remFileName)
        {
            ...
            {
                // Legend 2018/04/10 添加路徑
                LogFactory.Write("remotePath: " + remotePath + ",刪除FTP檔案 " + remFileName + " 失敗：" + ex.ToString(),
