...
namespace CIMSBatch.Mail
{
    class MailBase
    {
        ...
        private static void WriteErrLog(string ErrDes)
        {
            ...
            ErrStr.Append(";");
            ErrStr.Append("發送信息：");
            ErrStr.Append("伺服器 " + MailServer  + "|");
            ErrStr.Append("收件信箱 " + ManagerMail + "|");
            ErrStr.Append("發件信箱 " + MailFrom + "|");
            ErrStr.Append("發件賬號：" + MailUser + "|");
            ErrStr.Append("發件密碼：" + MailPassword);
